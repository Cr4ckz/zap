...
#import cetz.draw: (
  anchor, arc, arc-through, bezier, circle, content, line, merge-path, rect, rotate, set-origin, set-style,
)
...
/// *Anchors:* /// - `in1` to `in{n}`: Input pins (left).
/// - `out`: Output pin (right).
#let alogic(name: none, node: (0, 0), type: "and", inputs: 2, ..params) = logic_american(
  name: name,
  node: node,
  type: type,
  inputs: inputs,
